...
def extract_mbe(y, sr, n_fft, n_mels):
    # Perform STFT to get the spectrogram
    stft = librosa.stft(y, n_fft=n_fft, hop_length=n_fft//2)
    spec = np.abs(stft)**2  # power=1 is equivalent to magnitude squared
    
    # Convert the power spectrogram to a Mel spectrogram
    mel_spec = librosa.feature.melspectrogram(S=spec, sr=sr, n_fft=n_fft, n_mels=n_mels)
    return mel_spec
